...
class Validator {
  ...
  PromotePawn(pieceToPromoteTo) {
    ...
    const updatedBoardMap = helpers.updateBoardMap(pieceToPromoteTo, this.movingPiecesOrigin, this.movingPiecesDest, this.boardMap);
  ...
  newMove() {
    // Update the board map
    const updatedBoardMap = helpers.updateBoardMap(this.movingPiece, this.movingPiecesOrigin, this.movingPiecesDest, this.boardMap);
    this.boardMap = JSON.parse(JSON.stringify(updatedBoardMap));
    // Toggle the color's turn
    this.whitesTurn = !this.whitesTurn;
  }
  ...
  */
  ...
  validatePawnMove(origin, dest, color) {
    ...
    const [fileDifference, rankDifference] = helpers.getFileAndRankDifferences(originFile, originRank, destFile, destRank);
    // A Pawn cammot move diagonally or vertically more than 1 square
    if (fileDifference > 1 || rankDifference > 2) {
      return { isValid, isEnPassant, isPromotion };
    }
    let objectedPieceColor = '';
    isEnPassant = specials.isEnPassant(destFile, originRank, destRank, color, this.boardMap);
    ...
    if (color === 'w') {
      ...
      objectedPieceColor = helpers.checkThroughFile(originRank, destRank, originFile, this.boardMap).color;
  ...
  validateKnightMove(origin, dest, color) {
    ...
    // and get its color
    const destPieceColor = helpers.getPieceColor(this.boardMap[destFile][destRank]);
  ...
  validateKingMove(origin, dest, color) {
    ...
    const isCastle = specials.isCastle(origin, dest, color, this.boardMap, this.canWhiteCastleKingSide, this.canWhiteCastleQueenSide, this.canBlackCastleKingSide, this.canBlackCastleQueenSide);
  ...
  validateBishopMove(origin, dest, color) {
    ...
    const objectedSquareInfo = helpers.checkThroughDiagonals(originFile, destFile, originRank, destRank, this.boardMap);
  ...
  validateRookMove(origin, dest, color) {
    ...
    if (originRank === destRank) {
      // See if there is any piece on the way
      const objectedSquareInfo = helpers.checkThroughRank(originFile, destFile, originRank, this.boardMap);
    ...
    // If the move is along ranks (vertically, like a1 to a8)
      ...
      // See if there is any piece on the way
      const objectedSquareInfo = helpers.checkThroughFile(originRank, destRank, originFile, this.boardMap);
      // If the piece's color is same as the Rook
      // then the Rook cannot move through/to it
      if (color === objectedSquareInfo.color) {
        return false;
