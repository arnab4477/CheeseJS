 ...
 */
export const isSafe = (piece, origin, dest, whiteKingsPosition, blackKingsPosition, boardMap) => {
 ...
 */
export const checkVertically = (file, rank, color, direction, boardMap) => {
  let isCheck = false;
  ...
  if (direction === 'up') {
    ...
    if (color === 'w' && objectedSquareInfo.piece === 'K') {
      objectedSquareInfo = helpers.checkThroughFile((parseInt(rank) - 2).toString(), '1', file, boardMap);
    }
  ...
  if (helpers.isAdjacent(file + rank, objectedSquareInfo.square)) {
    ...
    isCheck = helpers.evaluateCheck(objectedSquareInfo.piece, color, objectedSquareInfo.color, ['Q', 'R', 'K'], ['q', 'r', 'k']);
    if (isCheck) {
      return true;
    }
  }
  // Check if any enemy Queen or Rook is on the way across squares
  isCheck = helpers.evaluateCheck(objectedSquareInfo.piece, color, objectedSquareInfo.color, ['Q', 'R'], ['q', 'r']);
  if (isCheck) {
    return true;
  }
 ...
 */
export const checkHorizontally = (file, rank, color, direction, boardMap) => {
  let isCheck = false;
  let objectedSquareInfo = {
    square: '',
    piece: '',
 ...
 */
export const pieceAt = (fileUnicode, rankNum, boardMap) => {
 ...
 * from any direction
 */
export const isCheckFromKnight = (file, rank, color, boardMap) => {
 ...
 * or "left-down"
 */
export const isCheckFromPawn = (color, pawn, direction) => {
 ...
 * or "left-down"
 */
export const checkDiagonally = (file, rank, color, direction, boardMap) => {
  // Get the diagonal edge for a given direction
  let [diagonalEdgeFile, diagonalEdgeRank] = helpers.getDiagonalEdge(file + rank, direction);
  let isCheck = false;
  let objectedSquareInfo = {
    square: '',
    piece: '',
  ...
  objectedSquareInfo = helpers.checkThroughDiagonals(file, diagonalEdgeFile, rank, diagonalEdgeRank, boardMap);
  // Check if the objected piece is on the adjaent square
  if (helpers.isAdjacent(file + rank, objectedSquareInfo.square)) {
    ...
    isCheck = helpers.evaluateCheck(objectedSquareInfo.piece, color, objectedSquareInfo.color, ['Q', 'B', 'K'], ['q', 'b', 'k']);
    if (isCheck) {
      return true;
    }
  }
  // Check if any enemy Queen or Bishop is on the way across squares
  isCheck = helpers.evaluateCheck(objectedSquareInfo.piece, color, objectedSquareInfo.color, ['Q', 'B'], ['q', 'b']);
  if (isCheck) {
    return true;
  }
  // If none of the checks returned true, then there is no check
 ...
 */
export const isCheck = (square, color, boardMap) => {
